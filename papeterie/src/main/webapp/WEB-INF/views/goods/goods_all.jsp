...
<div id="section-goods">
	<div id="goods-head">
		...
		<div id="goods-head-menu">
			<a href="#">추천순</a>
			<a href="#">인기순</a>
			<a href="goods_all?st=2">최신순</a>
			<a href="goods_all?st=3">낮은가격순</a>
			<a href="goods_all?st=4">높은가격순</a>
		</div>
	</div>
   	...
   	<ul class="paging">
   		<!-- 10페이지 이전 -->
   	 	<c:if test="${pstart != 1}">
   	 		<li>
   	 			<div class="pagingBox">
   	 				<a href="/papeterie/goods/goods_all?page=${pstart-10}&st=${st}">◀맨앞</a>
   	 			</div>
   	 		</li>
   	 	</c:if>
   	 	<c:if test="${pstart == 1}">
		...
		<c:if test="${page != 1 }">
   	 		<li>
   	 			<div class="pagingBox" style="margin-right:10px;">
   	 				<a href="/papeterie/goods/goods_all?page=${page-1}&st=${st}">◁이전</a>
   	 			</div>
   	 		</li>
   	 	</c:if>
   	 	<c:if test="${page == 1}">
   		...
   		<c:forEach begin="${pstart}" end="${pend}" var="i">
	   	 	...
	   	 	<c:if test="${page == i}">
	   	 		...
	   	 		<li><a id="pagingPage" href="/papeterie/goods/goods_all?page=${i}&st=${st}" ${str}>${i}</a></li>
	   	 	</c:forEach>
   		...
   		<!-- 다음페이지 -->	
   	 	<c:if test="${page != chong}">
  	 		<li>
  	 			<div class="pagingBox">
  	 				<a href="/papeterie/goods/goods_all?page=${page+1}&st=${st}">다음▷</a>
  	 			</div>
  	 		</li>
	 	</c:if>
  	 	<c:if test="${page == chong}">
  	 	...
  	 	<c:if test="${pend != chong}">
   	 		<li>
   	 			<div class="pagingBox">
   	 				<a href="/papeterie/goods/goods_all?page=${chong}&st=${st}">맨뒤▶</a>
   	 			</div>
   	 		</li>
   	 	</c:if>
   	 	<c:if test="${pend == chong}">
